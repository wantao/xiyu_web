...
require_once dirname(dirname(__FILE__)) . '/unity/self_account.php';
...
try {
	...
	if (!isset($_REQUEST['server_code'])) {
		exit("Url param error:".ErrorCode::URL_HAS_NO_SERVER_CODE);
	}	
    $act = isset($_REQUEST['act']) ? $_REQUEST['act'] : 'get_info';
    $data = processRequest($qihooOauth2, $act);
    $oa = new AccountOperation();
    //print_r($data);
    //$session_key = $oa->updateAccount($data['user']['id'], $data['access_token'], PLATFORM::THREE_SIX_ZERO);
    //echo $session_key;
    $login_server_info_and_session_key = array();
    $login_server_info_and_session_key = $oa->updateAccount($data['user_id'], $_REQUEST['token'], PLATFORM::THREE_SIX_ZERO, $_REQUEST['server_code']);
